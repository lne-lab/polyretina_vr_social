...
public class NPC_NavigationController : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject m_targetOfInterest;
    ...
    public void UpdateAnimatorBlendTree()
    {
       // m_animator.SetBool("Moving", Moving);
        //m_animator.SetFloat("Forward", m_velocity.x);
        m_animator.SetFloat("Forward", m_velocity.magnitude);
        m_animator.SetFloat("Right", m_velocity.y);
    }
    ...
    public void GoToObjectOfInterest(GameObject target)
    {
    ...
    public bool WithinRadiusOf(GameObject holdable)
    {
        ...
        float targetRadius = holdable == null ? 0f : holdable.transform.localScale.x;
